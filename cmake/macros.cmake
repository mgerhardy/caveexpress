...
macro(cp_add_executable)
	...
	set(_MULTI_VALUE_ARGS SRCS ASSETS)
	...
	if (ANDROID)
		add_library(${_EXE_TARGET} SHARED ${_EXE_SRCS} ${_EXE_ASSETS})
		...
		if (WINDOWED)
			if (WINDOWS)
				...
				add_executable(${_EXE_TARGET} WIN32 ${_EXE_SRCS} ${_EXE_ASSETS})
				if (MSVC)
					set_target_properties(${_EXE_TARGET} PROPERTIES LINK_FLAGS "/SUBSYSTEM:WINDOWS")
				endif()
			elseif (DARWIN OR IOS)
				add_executable(${_EXE_TARGET} MACOSX_BUNDLE ${_EXE_SRCS} ${_EXE_ASSETS})
				set_source_files_properties(${_EXE_ASSETS} PROPERTIES MACOSX_PACKAGE_LOCATION Resources)
				cp_osx_add_target_properties(${_EXE_TARGET} ${VERSION} ${VERSION_CODE})
				cp_osx_prepare_content(${_EXE_TARGET} ON)
			else()
				add_executable(${_EXE_TARGET} ${_EXE_SRCS} ${_EXE_ASSETS})
			endif()
		else()
			add_executable(${_EXE_TARGET} ${_EXE_SRCS} ${_EXE_ASSETS})
			if (DARWIN OR IOS)

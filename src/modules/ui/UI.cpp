...
bool UI::initLanguage (const std::string& language)
{
	...
	if (!buffer || fileLen <= 0) {
		Log::error(LOG_CLIENT, "could not load language " + language);
		return false;
	}
	...
	Log::info(LOG_CLIENT, "loaded language " + language + " with " + string::toString(_languageMap.size()) + " entries");
	return true;
}
...
const std::string UI::translate (const std::string& in) const
{
	...
	if (i == _languageMap.end()) {
		Log::error(LOG_CLIENT, "Missing translation for: " + in);
		return in;
	}
...
void UI::init (ServiceProvider& serviceProvider, EventHandler &eventHandler, IFrontend &frontend)
{
	...
	Log::info(LOG_CLIENT, "init the texture cache with " + serviceProvider.getTextureDefinition().getTextureSize());
...
bool UI::loadGesture (const unsigned char* data, int length)
{
	...
	if (n == -1) {
		...
		Log::error(LOG_CLIENT, "Failed to load gesture: " + e);
		return false;
	} else if (n == 0) {
...
void UI::onJoystickButtonPress (uint8_t button)
{
	...
	Log::debug(LOG_CLIENT, String::format("joystick button %i was pressed and not handled", (int)button));
}
...
void UI::onControllerButtonPress (const std::string& button)
{
	...
	Log::debug(LOG_CLIENT, "controller button " + button + " was pressed and not handled");
}
...
UIWindow* UI::push (const std::string& windowID)
{
	...
	if (!window) {
		Log::error(LOG_CLIENT, "could not find window '" + windowID + "'");
		return nullptr;
	}
	...
	Log::info(LOG_CLIENT, "push window " + windowID);
...
void UI::pop ()
{
	...
	Log::info(LOG_CLIENT, "pop window " + window->getId());
...
UINodeBar* UI::setBarValue (const std::string& window, const std::string& nodeId, uint16_t value)
{
	...
	if (!node) {
		Log::error(LOG_CLIENT, "could not get the node with the id " + nodeId + " from window " + window);
		return nullptr;
	}

...
bool SDLSoundEngine::init (bool initCache)
{
	if (SDL_InitSubSystem(SDL_INIT_AUDIO) == -1) {
		Log::error2(LOG_CLIENT, "unable to initialize audio: %s", SDL_GetError());
		_state = SOUND_CLOSED;
		return false;
	}
	const int n = SDL_GetNumAudioDrivers();
	if (n == 0) {
		Log::error2(LOG_CLIENT, " no built-in audio drivers");
		_state = SOUND_CLOSED;
		SDL_QuitSubSystem(SDL_INIT_AUDIO);
		return false;
	} else {
		for (int i = 0; i < n; ++i) {
			Log::info2(LOG_CLIENT, "available audio driver %s", SDL_GetAudioDriver(i));
		}
	}

	Log::info2(LOG_CLIENT, "actual audio driver: %s", SDL_GetCurrentAudioDriver());
	...
	if (Mix_OpenAudio(audioRate, audioFormat, audioChannels, audioBuffers) != 0) {
		Log::error2(LOG_CLIENT, "unable to initialize mixer: %s", Mix_GetError());
		_state = false;
		SDL_QuitSubSystem(SDL_INIT_AUDIO);
		return false;
	}
	...
	Log::info2(LOG_CLIENT, "sound initialized");
...
{
	Log::info2(LOG_CLIENT, "closing the sound system");
	Mix_HaltMusic();
	Mix_FreeMusic(_music);
	_music = nullptr;
	_musicPlaying = "";
...
int SDLSoundEngine::playMusic (const std::string& music, bool loop)
{
	...
	if (music.empty()) {
		Log::error2(LOG_CLIENT, "no music file to play was provided");
		return -1;
	}
	...
	if (rwops == nullptr) {
		Log::error2(LOG_CLIENT, "unable to load music file: %s", music.c_str());
		return -1;
	}
...
#endif
	if (_music == nullptr) {
		Log::error2(LOG_CLIENT, "unable to load music file: %s", Mix_GetError());
		return -1;
	}

	const int ret = Mix_PlayMusic(_music, loop ? -1 : 1);
	if (ret == -1)
		Log::error2(LOG_CLIENT, "unable to play music file: %s", Mix_GetError());
	else
		_musicPlaying = music;
	return ret;
}
...
int SDLSoundEngine::play (const std::string& filename, const vec2& position, bool loop)
{
	...
	if (filename.empty()) {
		Log::error2(LOG_CLIENT, "no sound file to play was provided");
		return -1;
	}
	...
	if (channel == -1) {
		Log::error2(LOG_CLIENT, "unable to play sound file: %s", Mix_GetError());
	} else {
		assert(channel >= 0);
		assert(channel < lengthofi(_channels));
...
bool SDLSoundEngine::cache (const std::string& sound)
{
	...
	Log::debug2(LOG_CLIENT, "cache sound %s", sound.c_str());
...
{
	if (filename.empty()) {
		Log::error2(LOG_CLIENT, "no sound file to get the chunk for was provided");
		return nullptr;
	}
	...
	_map[filename] = sound;
	if (sound == nullptr) {
		Log::error2(LOG_CLIENT, "unable to load sound file %s: %s", filename.c_str(), Mix_GetError());
	}
...
{
	if (!isActive()) {
		Log::info2(LOG_CLIENT, "sound is already paused");
		return;
	}
	Log::info2(LOG_CLIENT, "sound is now paused");
	_state |= SOUND_PAUSE;
	Mix_PauseMusic();
	Mix_Pause(-1);
}
...
{
	if (isActive()) {
		Log::info2(LOG_CLIENT, "sound is already active");
		return;
	}
	Log::info2(LOG_CLIENT, "sound is active again");
	_state &= ~SOUND_PAUSE;
	Mix_ResumeMusic();
	Mix_Resume(-1);
}

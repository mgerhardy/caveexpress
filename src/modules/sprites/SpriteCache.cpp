...
SpritePtr SpriteCache::load (const std::string& spriteName)
{
	...
	if (!def) {
		if (!sprite->addFrame(LAYER_DEFAULT, spriteName))
			Log::error(LOG_SPRITES, "could not add frame '%s' to sprite '%s' (def does not exist)", spriteName.c_str(), spriteName.c_str());
	} else {
		if (def->hasNoTextures()) {
			if (!sprite->addFrame(LAYER_DEFAULT, spriteName))
				Log::error(LOG_SPRITES, "could not add frame '%s' to sprite '%s' (def has no textures)", spriteName.c_str(), spriteName.c_str());
		} else {

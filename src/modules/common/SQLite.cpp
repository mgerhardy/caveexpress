...
bool SQLiteStatement::bindText (int index, const std::string& value)
{
	...
	Log::error(LOG_STORAGE, "can't bind text: " + value + " (" + _error + ")");
	return false;
}
...
bool SQLiteStatement::bindInt (int index, int value)
{
	...
	if (retVal == SQLITE_OK)
		return true;
	_error = sqlite3_errstr(retVal);
	Log::error(LOG_STORAGE, "can't bind int: " + string::toString(value) + " (" + _error + ")");
	return false;
}
...
int SQLiteStatement::step (bool reset)
{
	...
	if (retVal != SQLITE_DONE && retVal != SQLITE_ROW) {
		_error = sqlite3_errstr(retVal);
		Log::error(LOG_STORAGE, "could not step: " + _error);
	}
	...
	if (reset) {
		...
		if (resetRetVal != SQLITE_OK) {
			_error = sqlite3_errstr(retVal);
			Log::error(LOG_STORAGE, "could not reset: " + _error);
		}
	}
...
bool SQLite::open ()
{
	...
	if (rc != SQLITE_OK) {
		...
		Log::error(LOG_STORAGE, String::format("Can't open database '%s': %s", _fileName.c_str(), _error.c_str()));
		sqlite3_close(_db);
		_db = nullptr;
		return false;
	}
...
bool SQLite::prepare (SQLiteStatement& s, const std::string& statement)
{
	sqlite3_stmt *stmt;
	const char *pzTest;
	Log::debug(LOG_STORAGE, "Statement: " + statement);
	const int rc = sqlite3_prepare_v2(_db, statement.c_str(), statement.size(), &stmt, &pzTest);
	if (rc != SQLITE_OK) {
		_error = sqlite3_errmsg(_db);
		Log::error(LOG_STORAGE, "failed to prepare the insert statement: " + _error);
		s.init(nullptr);
		return false;
	}
...
bool SQLite::exec (const std::string& statement)
{
	...
	if (rc != SQLITE_OK) {
		if (zErrMsg != nullptr) {
			...
			Log::error(LOG_STORAGE, "SQL error: " + _error);
		}

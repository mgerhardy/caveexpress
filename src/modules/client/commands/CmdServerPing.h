...
public:
	...
	void run (const Args& args) override
	{
		...
		if (!_serviceProvider.getNetwork().discover(this, Config.getPort()))  {
			Log::error2(LOG_CLIENT, "could not send the ping broadcast");
		} else {
			Log::info2(LOG_CLIENT, "sent ping broadcast");
		}
	}
	...
	void onOOBData (const std::string& host, const IProtocolMessage* message) override
	{
		if (message->getId() != protocol::PROTO_PING) {
			Log::error2(LOG_CLIENT, "got invalid discover broadcast reply");
			return;
		}
		Log::info2(LOG_CLIENT, "got ping broadcast reply");

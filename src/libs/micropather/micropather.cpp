...
PathNode* PathNodePool::GetPathNode (unsigned frame, void* _state, float _costFromStart, float _estToGoal,
		PathNode* _parent)
{
...
int MicroPather::Solve (void* startNode, void* endNode, vector<void*>* path, float* cost)
{
	...
	*cost = 0.0f;
	...
	PathNode* newPathNode = pathNodePool.GetPathNode(frame, startNode, 0, graph->LeastCostEstimate(startNode, endNode),
			0);
			...
			*cost = node->costFromStart;
			...
			for (int i = 0; i < node->numAdjacent; ++i) {
				...
				if (inEither) {
					...
					child->parent = node;
					child->costFromStart = newCost;
					child->estToGoal = graph->LeastCostEstimate(child->state, endNode), child->CalcTotalCost();
...
int MicroPather::SolveForNearStates (void* startState, std::vector<StateCost>* near, float maxCost)
{
	...
	while (!open.Empty()) {
		...
		for (int i = 0; i < node->numAdjacent; ++i) {
			...
			// should never re-process the parent.

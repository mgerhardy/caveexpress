...
Sprite::Sprite (const std::string& name, const std::vector<int>& delays,
		const AnimationFrames& textures, int frameCount, float fps, int32_t frameTimeRemaining, bool loop,
		int spriteWidth, int spriteHeight) :
		_name(name), _delays(delays), _currentFrame(-1), _frameCount(frameCount), _fps(
...
Sprite* Sprite::copy () const
{
	...
	return new Sprite(_name, _delays, _textures, _frameCount, _fps, _frameTimeRemaining, _loop, _spriteWidth, _spriteHeight);
}
...
bool Sprite::addFrame (const std::string& name, int delay)
{
	if (name.empty())
		return false;
...
bool Sprite::render (IFrontend *frontend, int x, int y, int w, int h, int16_t angle, float alpha) const
{
	if (_currentFrame == -1)
		return false;
	// culling
...
bool Sprite::render (IFrontend *frontend, int y) const
{
	if (_currentFrame == -1)
		return false;
	// culling the sprites
...
bool Sprite::render (IFrontend *frontend, int x, int y, int16_t angle, float alpha) const
{
	if (_currentFrame == -1)
		return false;

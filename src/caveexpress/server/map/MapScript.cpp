...
int luaEntityRemove (lua_State* L)
{
	...
	if (entity->isNpcCave()) {
		INPCCave* caveNpc = dynamic_cast<INPCCave*>(entity);
		if (caveNpc != nullptr) {
...
int luaEntityResetAcceleration (lua_State* L)
{
	...
	Player* player = assert_cast<Player*, IEntity*>(entity);
	if (lua_gettop(L) >= 2)
		player->resetAcceleration(luaParseDirection(L, 2));
	else
		player->resetAcceleration(0);
	return 0;
}
...
int luaEntityLeavePackage (lua_State* L)
{
	...
	Package* package = packageNpc->leavePackage();
	luaPushEntity(L, package);
	return 1;
}
...
int luaEntityDropPackage (lua_State* L)
{
	...
	Package* package = packageNpc->dropPackage();
	luaPushEntity(L, package);
	return 1;
}
...
{
	IEntity* entity = luaGetEntity(L);
	CollectableEntity* collectable = dynamic_cast<CollectableEntity*>(entity);
	if (collectable == nullptr)
		luaL_error(L, "isCollected() is only valid for collectables");
	lua_pushboolean(L, collectable->isCollected());
	return 1;
}
...
{
	IEntity* entity = luaGetEntity(L);
	NPCFriendly* npc = dynamic_cast<NPCFriendly*>(entity);
	if (npc == nullptr)
	...
	if (lua_isnumber(L, 2)) {
		...
		cave = map->getCave(luaL_checkinteger(L, 2) - 1);
	} else {

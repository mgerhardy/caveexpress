...
void INPCCave::setPos (const b2Vec2& pos)
{
	...
	if (pos.x < getMaxWalkingLeft() || pos.x > getMaxWalkingRight())
		Log::error2(LOG_SERVER, "invalid position given");
	NPC::setPos(pos);
}
...
void INPCCave::moveAwayFromCave ()
{
	...
	if (fequals(leftTileX, rightTileX, 0.02f)) {
		Log::error2(LOG_SERVER, "move away from cave is not possible, there is not enough space");
	...
	Log::info2(LOG_SERVER, "moveTarget: %f, start: %f, end: %f, posX: %f", moveTargetX, start, end, posX);

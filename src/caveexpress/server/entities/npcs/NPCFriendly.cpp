...
bool NPCFriendly::updateCollectedState ()
{
	...
	for (Map::PlayerListConstIter i = players.begin(); i != players.end(); ++i) {
		...
		Log::debug(LOG_SERVER, String::format("landed on target cave and (re-)spawned npc with id %i", getID()));
		if (bonus) {
...
bool NPCFriendly::setArrived (const b2Vec2& targetPos)
{
	...
	Log::info(LOG_SERVER, String::format("took %ims to transfer the npc %i to its target cave", deltaSeconds, _id));

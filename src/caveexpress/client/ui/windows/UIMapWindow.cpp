...
UIMapWindow::UIMapWindow (IFrontend *frontend, ServiceProvider& serviceProvider, CampaignManager& campaignManager, ClientMap& map) :
		UIWindow(UI_WINDOW_MAP, frontend, WINDOW_FLAG_MODAL | WINDOW_FLAG_FULLSCREEN), _cursorActive(false), _serviceProvider(serviceProvider)
{
	...
	_nodeMap = new UINodeMap(frontend, serviceProvider, campaignManager, 0, 0, _frontend->getWidth(), _frontend->getHeight(), map);
...
bool UIMapWindow::isGameActive () const
{

...
template<typename T>
class ScopedArrayPtr: public NonCopyable {
public:
	explicit ScopedArrayPtr (T* const p = nullptr) :
			p_(p)
	{
	}
...
public:
	...
	SharedPtr () :
			_refCount(nullptr), _deletion(nullptr), _pointer(nullptr)
	{
	}
...
private:
	SharedPtr<T> _ptr;
public:
	explicit SharedPtrIf(const SharedPtr<T>& ptr) :
			_ptr(ptr) {
	}
	bool operator()(const SharedPtr<T>& other) const {
		return other.get() == _ptr.get();
	}
};

function getName()
	return "Letter4 T"
end

function onMapLoaded()
end

function initMap()
	-- get the current map context
	local map = Map.get()
	map:addTile("tile-background-03", 0.000000, 0.000000)
	map:addTile("tile-background-01", 0.000000, 1.000000)
	map:addTile("tile-background-01", 0.000000, 2.000000)
	map:addTile("tile-background-02", 0.000000, 3.000000)
	map:addTile("tile-background-01", 0.000000, 4.000000)
	map:addTile("tile-ground-01", 0.000000, 6.000000)
	map:addTile("tile-background-02", 0.000000, 7.000000)
	map:addTile("tile-ground-ledge-left-02", 0.000000, 8.000000)
	map:addTile("tile-background-02", 0.000000, 9.000000)
	map:addTile("tile-background-04", 1.000000, 0.000000)
	map:addTile("tile-background-02", 1.000000, 1.000000)
	map:addTile("tile-background-03", 1.000000, 2.000000)
	map:addTile("tile-rock-03", 1.000000, 3.000000)
	map:addTile("tile-background-02", 1.000000, 4.000000)
	map:addTile("tile-background-window-01", 1.000000, 5.000000)
	map:addTile("tile-ground-ledge-right-01", 1.000000, 6.000000)
	map:addTile("tile-ground-ledge-right-01", 1.000000, 8.000000)
	map:addTile("tile-background-04", 1.000000, 9.000000)
	map:addTile("tile-background-02", 2.000000, 0.000000)
	map:addTile("tile-background-04", 2.000000, 1.000000)
	map:addTile("tile-background-04", 2.000000, 2.000000)
	map:addTile("tile-background-01", 2.000000, 3.000000)
	map:addTile("tile-background-02", 2.000000, 4.000000)
	map:addTile("tile-background-02", 2.000000, 5.000000)
	map:addTile("tile-ground-06", 2.000000, 6.000000)
	map:addTile("tile-background-04", 2.000000, 7.000000)
	map:addTile("tile-ground-05", 2.000000, 8.000000)
	map:addTile("tile-background-03", 2.000000, 9.000000)
	map:addTile("tile-background-01", 3.000000, 0.000000)
	map:addTile("tile-background-04", 3.000000, 1.000000)
	map:addTile("tile-ground-01", 3.000000, 2.000000)
	map:addTile("tile-background-01", 3.000000, 3.000000)
	map:addTile("tile-background-01", 3.000000, 4.000000)
	map:addTile("tile-background-02", 3.000000, 5.000000)
	map:addTile("tile-background-04", 3.000000, 6.000000)
	map:addTile("tile-background-04", 3.000000, 7.000000)
	map:addTile("tile-background-04", 3.000000, 8.000000)
	map:addTile("tile-background-03", 3.000000, 9.000000)
	map:addTile("tile-background-02", 4.000000, 0.000000)
	map:addTile("tile-ground-03", 4.000000, 2.000000)
	map:addTile("tile-rock-right-04", 4.000000, 3.000000)
	map:addTile("tile-ground-02", 4.000000, 4.000000)
	map:addTile("tile-rock-right-04", 4.000000, 5.000000)
	map:addTile("tile-background-03", 4.000000, 6.000000)
	map:addTile("tile-rock-right-04", 4.000000, 7.000000)
	map:addTile("tile-ground-03", 4.000000, 8.000000)
	map:addTile("tile-ground-01", 4.000000, 9.000000)
	map:addTile("tile-background-03", 5.000000, 0.000000)
	map:addTile("tile-waterfall-01", 5.000000, 1.000000)
	map:addTile("tile-rock-left-04", 5.000000, 3.000000)
	map:addTile("tile-rock-left-04", 5.000000, 4.000000)
	map:addTile("tile-rock-left-04", 5.000000, 5.000000)
	map:addTile("tile-ground-02", 5.000000, 6.000000)
	map:addTile("tile-rock-left-04", 5.000000, 7.000000)
	map:addTile("tile-rock-left-04", 5.000000, 8.000000)
	map:addTile("tile-rock-slope-right-02", 5.000000, 9.000000)
	map:addTile("tile-background-01", 6.000000, 0.000000)
	map:addTile("tile-background-02", 6.000000, 1.000000)
	map:addTile("tile-ground-02", 6.000000, 2.000000)
	map:addTile("tile-background-03", 6.000000, 3.000000)
	map:addTile("tile-background-01", 6.000000, 4.000000)
	map:addTile("tile-rock-right-04", 6.000000, 5.000000)
	map:addTile("tile-background-04", 6.000000, 6.000000)
	map:addTile("tile-background-01", 6.000000, 7.000000)
	map:addTile("tile-background-01", 6.000000, 8.000000)
	map:addTile("tile-background-02", 6.000000, 9.000000)
	map:addTile("tile-background-02", 7.000000, 0.000000)
	map:addTile("tile-background-04", 7.000000, 1.000000)
	map:addTile("tile-background-01", 7.000000, 2.000000)
	map:addTile("tile-background-02", 7.000000, 3.000000)
	map:addTile("tile-background-01", 7.000000, 4.000000)
	map:addTile("tile-rock-01", 7.000000, 5.000000)
	map:addTile("tile-background-02", 7.000000, 6.000000)
	map:addTile("tile-ground-01", 7.000000, 8.000000)
	map:addTile("tile-background-02", 7.000000, 9.000000)
	map:addTile("tile-background-04", 8.000000, 0.000000)
	map:addTile("tile-background-02", 8.000000, 1.000000)
	map:addTile("tile-background-04", 8.000000, 2.000000)
	map:addTile("tile-background-01", 8.000000, 3.000000)
	map:addTile("tile-background-02", 8.000000, 4.000000)
	map:addTile("tile-ground-06", 8.000000, 5.000000)
	map:addTile("tile-background-01", 8.000000, 6.000000)
	map:addTile("tile-background-01", 8.000000, 7.000000)
	map:addTile("tile-background-03", 8.000000, 8.000000)
	map:addTile("bridge-wall-left-01", 8.000000, 8.000000)
	map:addTile("tile-background-03", 8.000000, 9.000000)
	map:addTile("tile-rock-slope-left-02", 9.000000, 0.000000)
	map:addTile("tile-background-03", 9.000000, 1.000000)
	map:addTile("tile-background-01", 9.000000, 2.000000)
	map:addTile("tile-background-03", 9.000000, 3.000000)
	map:addTile("tile-ground-ledge-right-01", 9.000000, 5.000000)
	map:addTile("tile-background-03", 9.000000, 6.000000)
	map:addTile("tile-background-01", 9.000000, 7.000000)
	map:addTile("tile-background-02", 9.000000, 8.000000)
	map:addTile("bridge-wall-right-01", 9.000000, 8.000000)
	map:addTile("tile-background-03", 9.000000, 9.000000)
	map:addTile("tile-rock-big-01", 10.000000, 0.000000)
	map:addTile("tile-background-02", 10.000000, 2.000000)
	map:addTile("tile-background-03", 10.000000, 3.000000)
	map:addTile("tile-background-03", 10.000000, 4.000000)
	map:addTile("tile-background-01", 10.000000, 5.000000)
	map:addTile("tile-background-04", 10.000000, 6.000000)
	map:addTile("tile-background-01", 10.000000, 7.000000)
	map:addTile("tile-rock-02", 10.000000, 8.000000)
	map:addTile("tile-background-04", 10.000000, 9.000000)
	map:addTile("tile-rock-slope-left-02", 11.000000, 2.000000)
	map:addTile("tile-background-04", 11.000000, 3.000000)
	map:addTile("tile-background-01", 11.000000, 4.000000)
	map:addTile("tile-background-04", 11.000000, 5.000000)
	map:addTile("tile-background-02", 11.000000, 6.000000)
	map:addTile("tile-background-03", 11.000000, 7.000000)
	map:addTile("tile-background-02", 11.000000, 8.000000)
	map:addTile("tile-background-04", 11.000000, 9.000000)

	map:addEmitter("item-stone", 1.000000, 2.000000, 1, 0, "")
	map:addEmitter("item-apple", 2.000000, 5.000000, 1, 0, "")
	map:addEmitter("item-apple", 6.000000, 1.000000, 1, 0, "")
	map:addEmitter("item-banana", 7.000000, 4.000000, 1, 0, "")
	map:addEmitter("tree", 9.000000, 6.000000, 1, 0, "")

	map:addCave("tile-cave-01", 0.000000, 5.000000, "", 2500)
	map:addCave("tile-cave-01", 1.000000, 7.000000, "", 500)
	map:addCave("tile-cave-02", 4.000000, 1.000000, "right=false", 3500)
	map:addCave("tile-cave-01", 7.000000, 7.000000, "right=false", 4000)
	map:addCave("tile-cave-02", 9.000000, 4.000000, "right=false", 5300)

	map:setSetting("width", "12")
	map:setSetting("height", "10")
	map:setSetting("fishnpc", "false")
	map:setSetting("flyingnpc", "false")
	map:setSetting("gravity", "9.81")
	map:setSetting("introwindow", "")
	map:setSetting("npcs", "5")
	map:setSetting("npctransfercount", "6")
	map:setSetting("packagetransfercount", "0")
	map:setSetting("points", "100")
	map:setSetting("referencetime", "130")
	map:setSetting("sideborderfail", "false")
	map:setSetting("theme", "rock")
	map:setSetting("tutorial", "false")
	map:setSetting("waterchangespeed", "0.000000")
	map:setSetting("waterfallingdelay", "4000")
	map:setSetting("waterheight", "0.900000")
	map:setSetting("waterrisingdelay", "0")
	map:setSetting("wind", "0")

	map:addStartPosition("2.000000", "7.000000")
end

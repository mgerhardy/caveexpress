...
wininstaller: strip
	$(Q)makensis -DPRODUCT_NAME=$(APPNAME) -DPRODUCT_VERSION=$(VERSION) -DAPPNAME=$(APPNAME) $(INSTALLER_DIR)/windows/setup.nsi
...
archives: strip
	...
	$(Q)GZIP=-9 tar -rf $(APPNAME)-$(MODE)-$(TARGET_OS)-$(TARGET_ARCH).tar.gz $(APPNAME)$(EXE_EXT)
	@echo "Create zip archive of $(APPNAME)"
	$(Q)git archive -9 --format zip -o $(APPNAME)-$(MODE)-$(TARGET_OS)-$(TARGET_ARCH).zip HEAD $(BASEDIR)
	$(Q)zip -qu9 $(APPNAME)-$(MODE)-$(TARGET_OS)-$(TARGET_ARCH).zip $(APPNAME)$(EXE_EXT)

...
$(ANDROID_PROJECT)/local.properties: $(ANDROID_PROJECT)/AndroidManifest.xml $(ANDROID_PROJECT)/build.xml $(ANDROID_PROJECT)/google-play-services_lib/build.xml
	...
	$(Q)cd $(ANDROID_PROJECT); SDK=`android list sdk --all | grep "SDK Platform Android 3.2" | awk -F'-' ' { print $$1 }'`; [ -n "$$SDK" ] && android update sdk -u -s -t $$SDK || echo
	$(Q)cd $(ANDROID_PROJECT); SDK=`android list sdk --all | grep "SDK Platform Android 4.1.2" | awk -F'-' ' { print $$1 }'`; [ -n "$$SDK" ] && android update sdk -u -s -t $$SDK || echo
	$(Q)cd $(ANDROID_PROJECT); SDK=`android list sdk --all | grep "Google Play Billing Library," | awk -F'-' ' { print $$1 }'`; [ -n "$$SDK" ] && android update sdk -u -s -t $$SDK || echo
	$(Q)cd $(ANDROID_PROJECT); SDK=`android list sdk --all | grep "Google Play services," | awk -F'-' ' { print $$1 }'`; [ -n "$$SDK" ] && android update sdk -u -s -t $$SDK || echo
